...
export default function AnnualCostDisplay({ plan, hasRxDrugs, hasDoctors }: Props) {
  ...
  return (
    <>
      <div style={{ margin:'0 12px 8px', padding:'11px 14px', borderRadius:'10px', backgroundColor:'#E6F7F9', border:'1px solid #E8E8E8' }} data-testid={`annual-cost-${plan.id}`}>
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:'10px' }}>
          <div style={{ flex:1 }}>
            <div style={{ display:'flex', alignItems:'center', gap:'6px', marginBottom:'2px' }}>
              <p style={{ fontSize:'10px', fontWeight:700, color:'#00353E', margin:0, textTransform:'uppercase', letterSpacing:'0.4px', fontFamily:"'Montserrat', sans-serif" }}>Estimated Annual Cost</p>
            ...
            <p style={{ fontSize:'22px', fontWeight:800, color:'#1A1A1A', margin:0, lineHeight:1, fontFamily:"'Montserrat', sans-serif" }}>
              {fmt(est.totalEstimate)}<span style={{ fontSize:'11px', fontWeight:400, color:'#8C8C8C', marginLeft:'4px' }}>/year</span>
            </p>
            <p style={{ fontSize:'10px', color:'#303030', margin:'4px 0 0', fontFamily:"'Montserrat', sans-serif" }}>
              {fmt(est.premiumAnnual)} premium + {fmt(est.oopEstimate)} est. out-of-pocket{est.isOopCappedByMoop ? ' (MOOP cap)' : ''}
            </p>
          </div>
          <button onClick={drawerOpen ? close : open} aria-label={`How we calculated the annual cost for ${plan.planName}`} style={{ background:'none', border:'1px solid #E8E8E8', borderRadius:'8px', cursor:'pointer', padding:'5px 9px', display:'flex', alignItems:'center', gap:'3px', color:'#00859A', fontSize:'10px', fontWeight:600, flexShrink:0, fontFamily:"'Montserrat', sans-serif" }}>
            <Info size={11} aria-hidden="true" /> How?
          </button>
        </div>
        {est.improvementHint && (
          <div style={{ marginTop:'7px', paddingTop:'7px', borderTop:'1px solid #E8E8E8', display:'flex', alignItems:'center', gap:'6px' }}>
            <p style={{ fontSize:'10px', color:'#303030', margin:0, flex:1, fontFamily:"'Montserrat', sans-serif" }}>{est.improvementHint}</p>
            <button onClick={handleImprove} style={{ background:'none', border:'none', cursor:'pointer', padding:0, display:'flex', alignItems:'center', gap:'3px', color:'#00859A', fontSize:'10px', fontWeight:700, textDecoration:'underline', fontFamily:"'Montserrat', sans-serif" }}>
      ...
      {drawerOpen && (
        <div style={{ position:'fixed', inset:0, zIndex:9999, display:'flex', alignItems:'flex-end', justifyContent:'center', backgroundColor:'rgba(11,27,36,0.45)' }} onClick={(e) => { if (e.target === e.currentTarget) close(); }}>
          <div style={{ width:'100%', maxWidth:'520px', backgroundColor:'#fff', borderRadius:'12px 12px 0 0', maxHeight:'80vh', overflowY:'auto', boxShadow:'0 -4px 32px rgba(11,27,36,0.14)' }}>
            <div style={{ backgroundColor:'#00353E', padding:'14px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', borderRadius:'12px 12px 0 0' }}>
              <div>
                <p style={{ color:'#fff', fontWeight:700, fontSize:'14px', margin:0, fontFamily:"'Montserrat', sans-serif" }}>How we calculated this</p>
                <p style={{ color:'rgba(255,255,255,0.6)', fontSize:'11px', margin:'2px 0 0', fontFamily:"'Montserrat', sans-serif" }}>{plan.planName}</p>
              </div>
            ...
            <div style={{ padding:'16px' }}>
              ...
              <div style={{ borderRadius:'10px', overflow:'hidden', border:'1px solid #E8E8E8', marginBottom:'12px' }}>
                {est.components.map((c, i) => (
                  <div key={c.id} style={{ padding:'10px 14px', borderBottom:i<est.components.length-1?'1px solid #F1F5F9':'none', backgroundColor:i%2===0?'#F9F9F9':'#fff', display:'flex', alignItems:'flex-start', gap:'10px' }}>
                    <div style={{ flex:1 }}>
                      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'baseline' }}>
                        <span style={{ fontSize:'12px', fontWeight:600, color:'#00353E', fontFamily:"'Montserrat', sans-serif" }}>{c.label}</span>
                        <span style={{ fontSize:'13px', fontWeight:800, color:'#1A1A1A', fontFamily:"'Montserrat', sans-serif" }}>{c.amount===0&&!c.isEstimated?'$0':`$${Math.round(c.amount).toLocaleString()}`}</span>
                      </div>
                      <p style={{ fontSize:'10px', color:'#8C8C8C', margin:'3px 0 0', lineHeight:1.4, fontFamily:"'Montserrat', sans-serif" }}>
              ...
              <p style={{ fontSize:'9px', color:'#8C8C8C', margin:'8px 0 0', lineHeight:1.4, fontFamily:"'Montserrat', sans-serif" }}>
