...
export default function MatchScoreBadge({ score, variant = 'full', onWhyClick }: Props) {
  ...
  if (variant === 'inline') {
    return (
      <span aria-label={`Match Score ${n} out of 100 — ${tier.label}`} style={{ fontSize:'13px', fontWeight:700, color:tier.color }}>
        {n}
        <span style={{ fontWeight:400, color:'#8C8C8C', marginLeft:'4px', fontSize:'11px' }}>— {tier.label}</span>
        {onWhyClick && (
          <button onClick={onWhyClick} aria-label="Why this score?" style={{ marginLeft:'6px', background:'none', border:'none', cursor:'pointer', fontSize:'11px', fontWeight:600, color:'#00859A', textDecoration:'underline', padding:0 }}>
            Why?
          </button>
        )}
      </span>
  ...
  return (
    <div style={{ display:'inline-flex', flexDirection:'column', alignItems:'center', padding:'8px 14px', borderRadius:'12px', backgroundColor:tier.bgColor, border:`1px solid ${tier.borderColor}`, minWidth:'80px', textAlign:'center' }} aria-label={`Match Score ${n} out of 100 — ${tier.label}`}>
      ...
      {onWhyClick && (
        <button onClick={onWhyClick} aria-label="Explain this match score" style={{ marginTop:'6px', background:'none', border:'none', cursor:'pointer', fontSize:'10px', fontWeight:600, color:'#00859A', textDecoration:'underline', padding:0 }}>
          Why this score?
        </button>
      )}
    </div>
